...
bool fill_and_present(brick::interfaces::display::IFrameBufferDisplay& framebuffers,
  ...
  if (!framebuffers.get_frame_buffer(index, buffer)) {
    board.logger().error(TAG, "Unable to acquire framebuffer %u", index);
    return false;
  }
  ...
  board.logger().info(TAG, "framebuffer=%u color=0x%04X fill=%lldus present+vsync=%lldus",
...
extern "C" void app_main() {
  board.logger().info(TAG, "Starting ESP32-S3 ST7701S framebuffer smoke test");
  if (!display.begin()) {
  ...
  board.logger().info(TAG, "Display initialized, framebuffer count=%u",
           framebuffers.frame_buffer_count());
  if (framebuffers.frame_buffer_count() < 2) {
    board.logger().error(TAG, "Two framebuffers are required for this test");
    return;
  }
