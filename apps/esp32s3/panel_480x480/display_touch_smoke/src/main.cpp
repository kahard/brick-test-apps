...
bool draw_marker(const brick::interfaces::display::TouchPoint& point, bool active)
{
    ...
    const brick::interfaces::display::PixelBuffer buffer{ reinterpret_cast<const std::uint8_t*>(marker.data()),
                                                          ...
                                                          brick::interfaces::display::PixelFormat::rgb565,
                                                          false };
    return screen.draw({ x0, y0, kMarkerSize, kMarkerSize }, buffer) && screen.wait_for_transfer_complete(1000);
}
}  // namespace
    ...
    while (true)
    {
        ...
        if (touch.read(points.data(), points.size(), count))
        {
            for (std::size_t index = 0; index < count; ++index)
            {
                ...
                const bool  moved =
                    active && (!marker_visible || marker_point.x != point.x || marker_point.y != point.y);
                if (!active)
                {
                    if (marker_visible)
                        draw_marker(marker_point, false);
                ...
                board.logger().info(TAG, "touch id=%u x=%d y=%d raw=(%d,%d) pressure=%d state=%u", point.id, point.x,
                                    point.y, point.raw_x, point.raw_y, point.pressure,
