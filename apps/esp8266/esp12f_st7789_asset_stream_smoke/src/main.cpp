...
brick::platform::esp8266::GpioButton       button(brick::platform::esp8266::profiles::esp12f_ttp223_gpio4());
...
bool show_asset(generated_assets::Id id, unsigned long& elapsed_us)
{
    ...
    const bool displayed =
        streamer.stream(*asset, asset_source, { 0, 0, kWidth, kHeight }, pixels.data(), pixels.size());
    ...
    if (pressed && !last_pressed)
    {
        ...
        else
        {
            Serial.printf("GPIO4 pressed; toggled asset=%s elapsed=%luus\n",
                          selected == generated_assets::Id::sweat_smile ? "sweat_smile" : "joy_tears", elapsed_us);
        }
    }
