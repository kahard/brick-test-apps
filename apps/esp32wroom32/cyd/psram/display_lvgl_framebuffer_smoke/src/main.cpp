...
namespace
{
constexpr char                                    TAG[]       = "brick_cyd_lvgl_fb";
constexpr std::size_t                             kImageBytes = 100U * 100U * 2U;
constexpr std::size_t                             kFrameBytes = 320U * 240U * 2U;
...
bool                                              load_asset(generated_assets::Id id)
{
    ...
    return assets_partition && asset && asset->size == kImageBytes
           && esp_partition_read(assets_partition, asset->offset, image_pixels, asset->size) == ESP_OK;
}
void clicked(lv_event_t*)
{
    selected = selected == generated_assets::Id::joy_tears ? generated_assets::Id::sweat_smile :
                                                             generated_assets::Id::joy_tears;
    if (load_asset(selected))
    {
        ++counter;
        lv_image_set_src(image, &image_dsc);
        lv_label_set_text_fmt(counter_label, "Zmiana: %u", static_cast<unsigned>(counter));
        ESP_LOGI(TAG, "partition asset switched");
    }
}
}  // namespace
extern "C" void app_main()
{
    ...
    image_pixels     = static_cast<std::uint8_t*>(heap_caps_malloc(kImageBytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    assets_partition = esp_partition_find_first(ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_ANY, "assets");
    if (image_pixels == nullptr || assets_partition == nullptr)
    {
        ESP_LOGE(TAG, "asset buffer or assets partition unavailable");
        return;
    }
    if (!image_pixels || !assets_partition || !display.begin() || !touch.begin() || !load_asset(selected))
    {
        ESP_LOGE(TAG, "PSRAM, partition, display, touch or asset initialization failed");
        return;
    }
    image_dsc = { .header    = { .cf = LV_COLOR_FORMAT_RGB565, .w = 100, .h = 100, .stride = 200 },
                  .data_size = kImageBytes,
                  .data      = image_pixels };
    lv_init();
    brick::platform::esp32::LvglDisplayAdapter adapter(display);
    auto* first  = heap_caps_malloc(kFrameBytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    auto* second = heap_caps_malloc(kFrameBytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (first == nullptr || second == nullptr)
    {
        ESP_LOGE(TAG, "framebuffer allocation failed");
        return;
    }
    if (!adapter.create(LV_DISPLAY_RENDER_MODE_FULL, first, second, kFrameBytes))
        return;
    brick::platform::esp32::LvglTouchAdapter touch_adapter(touch);
    if (touch_adapter.create() == nullptr)
    {
        ESP_LOGE(TAG, "LVGL touch input creation failed");
        return;
    }
    ...
    lv_image_set_src(image, &image_dsc);
    lv_obj_align(image, LV_ALIGN_TOP_MID, 0, 42);
    ...
    lv_label_set_text(label, "PRZELACZ");
    lv_obj_set_style_text_font(label, &brick_roboto_20, 0);
    lv_obj_center(label);
    counter_label = lv_label_create(screen);
    lv_label_set_text(counter_label, "Asset: joy_tears");
    lv_obj_set_style_text_font(counter_label, &brick_roboto_20, 0);
    lv_obj_align(counter_label, LV_ALIGN_BOTTOM_MID, 0, -88);
    ESP_LOGI(TAG, "LVGL initialized: partition assets, mode=FULL buffers=2");
    while (true)
    {
