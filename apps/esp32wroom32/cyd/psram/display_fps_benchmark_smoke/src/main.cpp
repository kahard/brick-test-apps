...
bool present(std::uint8_t* framebuffer, std::int64_t& transfer_us)
{
    ...
    for (std::uint16_t y = 0; y < kHeight; y += kStripeHeight) {
        ...
        const brick::interfaces::display::PixelBuffer buffer{
            stripe, kWidth, height, static_cast<std::size_t>(kWidth) * kBytesPerPixel,
...
{
    ESP_LOGI(TAG, "Starting CYD PSRAM full-frame FPS benchmark: %ux%u frame_bytes=%u",
             kWidth, kHeight, static_cast<unsigned>(kFrameBytes));
    ...
    ESP_LOGI(TAG, "buffers_ready free_psram_after=%u", static_cast<unsigned>(heap_caps_get_free_size(MALLOC_CAP_SPIRAM)));
