...
void draw_test_pattern()
{
    ...
    constexpr std::uint16_t colors[] = { 0xF800, 0x07E0, 0x001F, 0xFFE0, 0xF81F, 0x07FF,
                                         0xFFFF, 0x0000, 0x7BEF, 0x4208, 0xFD20, 0xAFE5 };
...
extern "C" void app_main()
{
    ...
    while (true)
    {
        ...
        if (touch.read(points.data(), points.size(), count) && count != 0)
        {
            ...
            ESP_LOGI(TAG, "touch x=%d y=%d raw=(%d,%d) pressure=%d state=%u", point.x, point.y, point.raw_x,
                     point.raw_y, point.pressure, static_cast<unsigned>(point.state));
        }
