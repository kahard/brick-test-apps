...
bool load_asset(generated_assets::Id id, std::uint8_t* destination)
{
    ...
    if (asset == nullptr)
        return false;
    const brick::interfaces::display::WritablePixelBuffer buffer{ destination,
                                                                  kWidth,
                                                                  kHeight,
...
extern "C" void app_main()
{
    ...
    ESP_LOGI(TAG, "frame_bytes=%u double_buffer_bytes=%u free_psram=%u", static_cast<unsigned>(kFrameBytes),
             static_cast<unsigned>(2 * kFrameBytes), static_cast<unsigned>(heap_caps_get_free_size(MALLOC_CAP_SPIRAM)));
