...
{
    ESP_LOGI(TAG, "Starting CYD internal-RAM stripe asset/touch test: %ux%u stripe_bytes=%u", kWidth, kHeight,
    ...
    while (true)
    {
        ...
        while (xTaskGetTickCount() < deadline)
        {
            ...
            if (pressed && !was_pressed)
            {
                ...
                ESP_LOGI(TAG, "touch pressed at (%d,%d); next asset=%s", point.x, point.y,
