...
extern "C" void app_main()
{
    ...
    if (!assets_partition || !display.begin() || !touch.begin() || !load_asset(selected))
    {
        ESP_LOGE(TAG, "partition, display, touch or asset initialization failed");
        return;
    }
    lv_init();
    brick::platform::esp32::LvglDisplayAdapter adapter(display);
    if (!adapter.create(LV_DISPLAY_RENDER_MODE_PARTIAL, draw_buffer, nullptr, sizeof(draw_buffer)))
        return;
    ...
    lv_label_set_text(title, "CYD LVGL ASSETS FLASH");
