...
bool load(generated_assets::Id id, std::uint8_t* target)
{
    ...
    return assets && target && a && a->size == IMAGE_BYTES
           && esp_partition_read(assets, a->offset, target, a->size) == ESP_OK;
}
...
void run_asset_benchmark(lv_display_t* d)
{
    ...
    ESP_LOGI(TAG, "asset benchmark: mode=preloaded frames=%u elapsed=%lldus avg_frame=%lldus fps=%.2f", count,
             static_cast<long long>(elapsed), static_cast<long long>(elapsed / count), count * 1000000.0 / elapsed);
}
...
void run_color_benchmark(lv_display_t* d, lv_obj_t* screen)
{
    ...
    const auto elapsed = esp_timer_get_time() - start;
    ESP_LOGI(TAG, "color benchmark: mode=full_frame frames=%u elapsed=%lldus avg_frame=%lldus fps=%.2f", count,
             static_cast<long long>(elapsed), static_cast<long long>(elapsed / count), count * 1000000.0 / elapsed);
}
}  // namespace
...
extern "C" void app_main()
{
    ...
    if (!assets || !image_joy_tears || !image_sweat_smile || !first || !second || !board.begin())
    {
    ...
    {
        ESP_LOGE(TAG, "DMA scanout capability check failed: dma=%d vsync=%d scanout=%d buffers=%u", capabilities.dma,
                 capabilities.vsync, capabilities.scanout_buffers, capabilities.max_buffer_count);
        return;
    }
    ESP_LOGI(TAG, "DMA scanout ready: dma=%d dma_alignment=%u buffers=%u vsync=%d dma2d=1", capabilities.dma,
             static_cast<unsigned>(capabilities.dma_alignment_bytes), capabilities.max_buffer_count,
             capabilities.vsync);
    if (!load(generated_assets::Id::joy_tears, image_joy_tears)
        || !load(generated_assets::Id::sweat_smile, image_sweat_smile))
    {
        ESP_LOGE(TAG, "asset preload failed");
        return;
    }
