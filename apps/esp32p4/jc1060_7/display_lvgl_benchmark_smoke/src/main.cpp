...
bool load(generated_assets::Id id) {
    const auto* a = generated_assets::find(id);
    return assets && a && a->size == IMAGE_BYTES && esp_partition_read(assets, a->offset, image, a->size) == ESP_OK;
}
...
void run_asset_benchmark(lv_display_t* d) {
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
        load((i & 1) ? generated_assets::Id::sweat_smile : generated_assets::Id::joy_tears);
        lv_image_set_src(image_widget, &image_dsc);
        lv_refr_now(d);
    }
    const auto elapsed = esp_timer_get_time() - start;
    ESP_LOGI(TAG, "asset benchmark: frames=%u elapsed=%lldus fps=%.2f", count, static_cast<long long>(elapsed), count * 1000000.0 / elapsed);
}
...
void run_color_benchmark(lv_display_t* d, lv_obj_t* screen) {
    ...
    const auto elapsed = esp_timer_get_time() - start;
    ESP_LOGI(TAG, "color benchmark: frames=%u elapsed=%lldus fps=%.2f", count, static_cast<long long>(elapsed), count * 1000000.0 / elapsed);
}
}
...
extern "C" void app_main() {
    ...
    image = static_cast<std::uint8_t*>(heap_caps_malloc(IMAGE_BYTES, caps));
    auto* first = heap_caps_malloc(FRAME_BYTES, caps);
    auto* second = heap_caps_malloc(FRAME_BYTES, caps);
    if (!assets || !image || !first || !second || !display.begin()) { ESP_LOGE(TAG, "initialization failed"); return; }
    gpio_set_direction(GPIO_NUM_23, GPIO_MODE_OUTPUT);
    gpio_set_level(GPIO_NUM_23, 1);
    image_dsc = {.header = {.cf = LV_COLOR_FORMAT_RGB565, .w = 100, .h = 100, .stride = 200}, .data_size = IMAGE_BYTES, .data = image};
    lv_init();
    ...
    if (!d || !load(generated_assets::Id::joy_tears)) { ESP_LOGE(TAG, "LVGL or asset initialization failed"); return; }
