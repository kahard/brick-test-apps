...
bool load_asset(generated_assets::Id id)
{
    ...
    return assets_partition && image_pixels && asset && asset->size == kImageBytes
...
void clicked(lv_event_t*)
{
    ...
    if (load_asset(showing_joy_tears ? generated_assets::Id::joy_tears : generated_assets::Id::sweat_smile))
    {
        ...
        lv_label_set_text_fmt(status_label, "%s | Dotkniecia: %u", showing_joy_tears ? "joy_tears" : "sweat_smile",
...
extern "C" void app_main()
{
    ...
    if (!assets_partition || !image_pixels || !first || !second)
    {
        ESP_LOGE(TAG, "asset partition or PSRAM allocation failed");
        return;
    }
    if (!board.begin() || !load_asset(generated_assets::Id::joy_tears))
    {
        ESP_LOGE(TAG, "board or asset initialization failed");
        return;
    }
    ...
    lv_label_set_text(title, "JC1060 LVGL ASSET");
    ...
    lv_label_set_text(status_label, "Asset z partycji flash");
    lv_obj_align(status_label, LV_ALIGN_BOTTOM_MID, 0, -110);
    ESP_LOGI(TAG, "LVGL initialized: mode=FULL framebuffer_bytes=%u asset_bytes=%u", static_cast<unsigned>(kFrameBytes),
