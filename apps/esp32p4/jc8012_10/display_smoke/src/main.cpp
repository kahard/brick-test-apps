...
constexpr std::uint16_t kLogicalHeight = (BRICK_PANEL_ROTATION == 90 || BRICK_PANEL_ROTATION == 270) ? 800 : 1280;
...
void draw_test_pattern()
{
    ...
    auto*      frame        = static_cast<std::uint16_t*>(
        heap_caps_calloc(frame_pixels, sizeof(std::uint16_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    if (frame == nullptr)
    {
        ESP_LOGE(TAG, "unable to allocate logical frame (%zu bytes)", frame_pixels * sizeof(std::uint16_t));
        return;
    }
    constexpr std::uint16_t colors[]      = { 0xF800, 0x07E0, 0x001F, 0xFFE0, 0xF81F, 0x07FF, 0x0000, 0xFFFF };
    ...
    for (std::uint16_t y = 0; y < kMarkerSize; ++y)
        for (std::uint16_t x = 0; x < kMarkerSize; ++x)
            frame[static_cast<std::size_t>(kLogicalHeight - kMarkerSize + y) * kWidth + x] = 0x0000;
    const brick::interfaces::display::PixelBuffer buffer{
        reinterpret_cast<const std::uint8_t*>(frame),    kWidth, kLogicalHeight, static_cast<std::size_t>(kWidth) * 2,
        brick::interfaces::display::PixelFormat::rgb565, false
    };
    const bool submitted = display.draw_buffer({ 0, 0, kWidth, kLogicalHeight }, buffer);
    ESP_LOGI(TAG, "full logical frame submitted=%d", submitted ? 1 : 0);
    if (submitted)
        display.wait_for_transfer_complete(2000);
    ...
    for (std::uint16_t index = 0; index < 8; ++index)
    {
        ...
        const bool                                    submitted =
            display.draw_buffer({ 0, static_cast<std::int16_t>(index * kStripeHeight), kWidth, kStripeHeight }, buffer);
    ...
    marker.fill(0x0000);
    const brick::interfaces::display::PixelBuffer marker_buffer{ reinterpret_cast<const std::uint8_t*>(marker.data()),
                                                                 kMarkerSize,
                                                                 kMarkerSize,
                                                                 static_cast<std::size_t>(kMarkerSize) * 2,
                                                                 brick::interfaces::display::PixelFormat::rgb565,
                                                                 false };
    const bool                                    marker_submitted = display.draw_buffer(
        { 0, static_cast<std::int16_t>(kLogicalHeight - kMarkerSize), kMarkerSize, kMarkerSize }, marker_buffer);
    ESP_LOGI(TAG, "orientation marker bottom_left submitted=%d", marker_submitted ? 1 : 0);
    if (marker_submitted)
        display.wait_for_transfer_complete(1000);
#endif
}
...
{
    ESP_LOGI(TAG, "Starting JC8012 10-inch JD9365 smoke test: logical=%ux%u rotation=%d", kWidth, kLogicalHeight,
