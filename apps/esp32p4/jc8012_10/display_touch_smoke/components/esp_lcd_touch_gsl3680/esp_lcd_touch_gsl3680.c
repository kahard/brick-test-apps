...
static bool      esp_lcd_touch_gsl3680_get_xy(esp_lcd_touch_handle_t tp, uint16_t* x, uint16_t* y, uint16_t* strength,
...
esp_err_t esp_lcd_touch_new_i2c_gsl3680(esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t* config,
                                        esp_lcd_touch_handle_t* out_touch)
{
    ...
    esp_lcd_touch_io_gsl3680_config_t* gsl3680_config =
        (esp_lcd_touch_io_gsl3680_config_t*)esp_lcd_touch_gsl3680->config.driver_data;
    ...
    if (gsl3680_config && esp_lcd_touch_gsl3680->config.rst_gpio_num != GPIO_NUM_NC
        && esp_lcd_touch_gsl3680->config.int_gpio_num != GPIO_NUM_NC)
    {
        ...
        ESP_RETURN_ON_ERROR(
            gpio_set_level(esp_lcd_touch_gsl3680->config.rst_gpio_num, esp_lcd_touch_gsl3680->config.levels.reset), TAG,
            "GPIO set level error!");
        ESP_RETURN_ON_ERROR(gpio_set_level(esp_lcd_touch_gsl3680->config.int_gpio_num, 0), TAG,
                            "GPIO set level error!");
        ...
        ESP_RETURN_ON_ERROR(gpio_set_level(esp_lcd_touch_gsl3680->config.int_gpio_num, gpio_level), TAG,
                            "GPIO set level error!");
        vTaskDelay(pdMS_TO_TICKS(1));

        ESP_RETURN_ON_ERROR(
            gpio_set_level(esp_lcd_touch_gsl3680->config.rst_gpio_num, !esp_lcd_touch_gsl3680->config.levels.reset),
    ...
    if (esp_lcd_touch_gsl3680->config.int_gpio_num != GPIO_NUM_NC)
    {
        ...
        {
            esp_lcd_touch_register_interrupt_callback(esp_lcd_touch_gsl3680,
...
static esp_err_t esp_lcd_touch_gsl3680_read_data(esp_lcd_touch_handle_t tp)
{
    ...
    //       __func__, i, XY_Coordinate[i].x_position, XY_Coordinate[i].y_position,
    //       XY_Coordinate[i].finger_id,Finger_num);
    // i=1;
    // printf("%s: %d[i], %d[x_position], %d[y_position], %d[finger_id], %d[finger_num]\n",
    //       __func__, i, XY_Coordinate[i].x_position, XY_Coordinate[i].y_position,
    //       XY_Coordinate[i].finger_id,Finger_num);
...
static esp_err_t esp_lcd_touch_gsl3680_read_ram_fw(esp_lcd_touch_handle_t tp)
{
    ...
    ESP_LOGI(TAG, "gsl3680 startup_chip failed read 0xb0 = %x,%x,%x,%x ", read_buf[3], read_buf[2], read_buf[1],
             read_buf[0]);
