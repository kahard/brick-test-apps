...
void touch_asset_event(lv_event_t*)
{
    ...
    ++changes;
    set_asset(showing_joy_tears ? generated_assets::Id::joy_tears : generated_assets::Id::sweat_smile);
    ESP_LOGI(kTag, "touch asset=%s changes=%u", showing_joy_tears ? "joy_tears" : "sweat_smile",
...
bool load_asset(generated_assets::Id id, std::uint8_t* target)
{
    ...
    return assets_partition && target && asset && asset->size == kImageBytes
           && esp_partition_read(assets_partition, asset->offset, target, asset->size) == ESP_OK;
}
...
void set_asset(generated_assets::Id id)
{
    ...
    lv_label_set_text_fmt(status_label, "Asset z flash | zmiany: %u", static_cast<unsigned>(changes));
#endif
}
...
bool prefetch_asset(generated_assets::Id id)
{
    ...
    if (descriptor == nullptr || asset_source == nullptr || asset_streamer == nullptr
        || !asset_streamer->stream_to_buffer(*descriptor, *asset_source,
...
{
    ESP_LOGI(kTag, "Starting JC8012 10-inch LVGL full-frame asset test: %ux%u rotation=%d frame_bytes=%u", kWidth,
             kHeight, BRICK_PANEL_ROTATION, static_cast<unsigned>(kFrameBytes));
    ...
    {
        ESP_LOGE(kTag, "DMA scanout capability check failed: dma=%d vsync=%d scanout=%d buffers=%u", capabilities.dma,
                 capabilities.vsync, capabilities.scanout_buffers, capabilities.max_buffer_count);
        return;
    }
    ESP_LOGI(kTag, "DMA scanout ready: dma=%d dma_alignment=%u buffers=%u vsync=%d dma2d=1", capabilities.dma,
             static_cast<unsigned>(capabilities.dma_alignment_bytes), capabilities.max_buffer_count,
             capabilities.vsync);
    ...
    if (!adapter.create(LV_DISPLAY_RENDER_MODE_FULL, first, nullptr, static_cast<std::uint32_t>(kFrameBytes)))
    {
...
#    ifdef BRICK_STREAM_TEST
        if (tick == 49)
        {
            const auto next = showing_joy_tears ? generated_assets::Id::sweat_smile : generated_assets::Id::joy_tears;
...
#        ifdef BRICK_COLOR_ONLY
            ...
            ++changes;
            lv_obj_set_style_bg_color(screen, showing_joy_tears ? lv_color_hex(0xE00000) : lv_color_hex(0x0000E0), 0);
            ESP_LOGI(kTag, "color=%s changes=%u", showing_joy_tears ? "red" : "blue", static_cast<unsigned>(changes));
#        else
            showing_joy_tears = !showing_joy_tears;
            ++changes;
            set_asset(showing_joy_tears ? generated_assets::Id::joy_tears : generated_assets::Id::sweat_smile);
            ESP_LOGI(kTag, "asset=%s changes=%u", showing_joy_tears ? "joy_tears" : "sweat_smile",
                     static_cast<unsigned>(changes));
#        endif
#    endif
        }
