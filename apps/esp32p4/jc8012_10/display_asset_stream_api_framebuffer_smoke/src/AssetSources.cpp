...
public:
    ...
    bool begin()
    {
        ...
        // Reject an incomplete or foreign bundle before rendering its first frame.
        std::uint8_t byte = 0;
        if (!file_ || !file_->seek(static_cast<long>(generated_assets::bundle_size - 1U), SEEK_SET)
...
struct AssetSources::Impl
{
    explicit Impl(brick::platform::esp32::SdmmcFileSystem& card)
        : sdmmc(card), sd_source(card), repository(&flash_source, &psram_source, &sd_source)
    {
    }
    ...
    brick::platform::esp32::SdmmcFileSystem&     sdmmc;
...
AssetSources::AssetSources(brick::platform::esp32::SdmmcFileSystem& sdmmc) : impl_(std::make_unique<Impl>(sdmmc))
{
}
...
brick::interfaces::display::IAssetSource* AssetSources::select(std::uint8_t storage)
{
    ...
    if (storage == 2 && !impl_->sd_ready)
    {
        if (!impl_->sdmmc.mount() || !impl_->sd_source.begin())
        {
            impl_->close_sd();
            return nullptr;
        }
